...
- 셰이더 동작 순서
  ...
  *흩뜨려 놓은 노이즈 텍스처를 다시 사용해 알파 텍스처를 샘플링하면 알파 텍스처를 타오르는 불꽃처럼 보이게 만들 수 있다.
  *샘플링 시 wrap대신 clamp를 사용해 불꽃의 바닥 주위에서 텍스처의 위쪽을 사용하는 것을 방지해야 한다.

  5) 이펙트를 완성하기 위해 만들어낸 알파 값을 불꽃 텍스처 사각형의 알파 채널에 사용하여 알파블렌딩으로 마무리될 수 있게 한다.
------------------------------------------------------------------------------------------------------------------------*/
...
public:
	...
	bool Render( int, D3DXMATRIX, D3DXMATRIX, D3DXMATRIX, ID3D11ShaderResourceView*, ID3D11ShaderResourceView*, ID3D11ShaderResourceView*,
...
private:
	...
	void ShutdownShader();
	bool SetShaderParameters(D3DXMATRIX, D3DXMATRIX, D3DXMATRIX, ID3D11ShaderResourceView*, ID3D11ShaderResourceView*,
