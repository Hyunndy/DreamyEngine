...
TerrainShaderClass2::TerrainShaderClass2()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
TerrainShaderClass2::TerrainShaderClass2(const TerrainShaderClass2&)
{
}
...
bool TerrainShaderClass2::Initialize(HWND hwnd)
{
	//정점 셰이더, 픽셀 셰이더 초기화
	return InitializeShader(hwnd, L"../Dreamy/shader/shadowterrain.vs", L"../Dreamy/shader/shadowterrain.ps");
...
-----------------------------------------------------------------------------------------------*/
bool TerrainShaderClass2::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "ShadowTerrainVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "ShadowTerrainPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL,
...
- 픽셀 셰이더에 텍스처를 설정한다.
-----------------------------------------------------------------------------------------------*/
bool TerrainShaderClass2::SetShaderParameters(int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	D3DXMATRIX projectionMatrix, D3DXMATRIX lightViewMatrix, D3DXMATRIX lightProjectionMatrix,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, ID3D11ShaderResourceView* depthMapTexture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor,
...

{
	...
	D3D::GetDeviceContext()->PSSetShaderResources(2, 1, &depthMapTexture);
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr4 = (LightBufferType2*)mappedResource.pData;

	// Copy the lighting variables into the constant buffer.
	dataPtr4->lightPosition = lightDirection;
...
bool TerrainShaderClass2::Render(int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	D3DXMATRIX projectionMatrix, D3DXMATRIX lightViewMatrix, D3DXMATRIX lightProjectionMatrix,
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture2, ID3D11ShaderResourceView* depthMapTexture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor,
	float fogStart, float fogEnd)
{
	// 렌더링에 사용할 셰이더 매개 변수를 설정합니다.
	if (!SetShaderParameters(indexCount, worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix,lightProjectionMatrix,texture, texture2, depthMapTexture,
		lightDirection, diffuseColor, fogStart, fogEnd))
	{
		return false;
	}
...
void TerrainShaderClass2::RenderShader(int indexCount)
{
...
void TerrainShaderClass2::ShutdownShader()
{
