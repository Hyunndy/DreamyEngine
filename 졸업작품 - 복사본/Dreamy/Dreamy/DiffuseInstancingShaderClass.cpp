...
DiffuseInstancingShaderClass::DiffuseInstancingShaderClass()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
DiffuseInstancingShaderClass::DiffuseInstancingShaderClass(const DiffuseInstancingShaderClass& other)
{
}
...
bool DiffuseInstancingShaderClass::Initialize(HWND hwnd)
{
	bool result;

	//2. 텍스처 붙어있는 삼각형 render하는 셰이더
	result = InitializeShader(hwnd, L"../Dreamy/shader/diffuseinstancing.vs", L"../Dreamy/shader/diffuseinstancing.ps");
...
bool DiffuseInstancingShaderClass::Render(int vertexCount, int instanceCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
*/
bool DiffuseInstancingShaderClass::InitializeShader(HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "DiffuseInstancingVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "DiffuseInstancingPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
...
void DiffuseInstancingShaderClass::ShutdownShader()
{
...
bool DiffuseInstancingShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
...
void DiffuseInstancingShaderClass::RenderShader(int vertexCount, int instanceCount)
{
