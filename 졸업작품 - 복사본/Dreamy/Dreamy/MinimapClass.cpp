...
bool MinimapClass::Initialize( int screenWidth, int screenHeight,
	float terrainWidth, float terrainHeight)
{
	...
	result = m_MiniMapBitmap->Initialize( screenWidth, screenHeight, L"../Dreamy/Data/minimap.png", 200, 200);
	if (!result)
	{
		return false;
	}
	...
	result = m_PointBitmap->Initialize( screenWidth, screenHeight, L"../Dreamy/Data/point.png", 3, 3);
	if (!result)
	{
		return false;
	}
...
bool MinimapClass::Render(TextureShaderClass* ShaderManager, D3DXMATRIX worldMatrix,
	D3DXMATRIX viewMatrix, D3DXMATRIX orthoMatrix)
{
	...
	result = ShaderManager->Render( m_MiniMapBitmap->GetIndexCount(), worldMatrix, viewMatrix,
	...
	result = m_PointBitmap->Render(m_pointLocationX, m_pointLocationY);
	if (!result) { return false; }

	// Render the point bitmap using the texture shader.
	result = ShaderManager->Render( m_PointBitmap->GetIndexCount(), worldMatrix, viewMatrix,
