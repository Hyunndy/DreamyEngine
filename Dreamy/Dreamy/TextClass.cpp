	...
	D3DXMATRIX baseViewMatrix)
{
	...
	// 사용할 문장을 업데이트 한다.
	result = UpdateSentence(m_sentence1, "Hello", 740, 10, 0.0f, 1.0f, 1.0f, deviceContext);
	if (!result) { return false;}
...
bool TextClass::SetFps(int fps, ID3D11DeviceContext* deviceContext)
{
	...
	//
	result = UpdateSentence(m_sentence2, fpsString, 740, 30, red, green, blue, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetCpu(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence3, cpuString, 740, 50, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
